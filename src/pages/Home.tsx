...
const RIGHT_SLOGANS = [
  ...
  'Create. Trade. Thrive.',
  'Luxury x Culture x Web3',
  'Community-owned economy.',
];

/** بارون کلمه‌ای – می‌تونی density/speed رو تنظیم کنی */
function MatrixRain({
  side,
  words,
  speed = 60,     // ms per frame (کم‌تر = سریع‌تر)
  density = 1.0,  // 0.5 خلوت‌تر، 1 پیش‌فرض، 1.5 شلوغ‌تر
  fixed = false,  // true = تمام‌صفحه ثابت (روی همهٔ سکشن‌ها)
}: {
  side: 'left' | 'right';
  words: string[];
  speed?: number;
  ...
  useEffect(() => {
    ...
    const tick = (now: number) => {
      if (!last || now - last >= speed) {
        last = now;
        // محو آهسته جهت رد شبحی
        ctx.fillStyle = 'rgba(0,0,0,0.15)';
        ctx.fillRect(0, 0, canvas.width, canvas.height);

        ctx.font = `${fontSize}px monospace`;
        ctx.fillStyle = '#7affc4';
        ctx.shadowColor = 'rgba(0,255,170,.55)';
        ctx.shadowBlur = 8;
  ...
  return (
    <div
      className={`pointer-events-none ${fixed ? 'fixed' : 'absolute'} inset-y-0 ${
        side === 'left' ? 'left-0' : 'right-0'
      } z-30 hidden md:block w-[min(22vw,280px)]`}
    >
      {/* بک‌گراند شیشه‌ای ملایم */}
      <div className="absolute inset-0 bg-black/35 backdrop-blur-sm rounded-xl" />
      <canvas ref={canvasRef} className="relative h-full w-full rounded-xl" />
    </div>
  );
}
    ...
    <section ref={wrapRef} className="relative h-[420vh] w-full overflow-visible">
      <div className="sticky top-0 h-[100svh] w-full">
        <div className="relative h-full w-full">
          ...
          {/* (برای بخش ویدیو هم می‌تونی رین محلی بزاری؛ اما چون رینِ ثابت داریم، نیاز نیست) */}
...
const Home = () => {
  ...
  return (
    <div className="min-h-screen relative">
      {/* رینِ ثابتِ تمام‌صفحه (کناره‌های سایت تا انتها) */}
